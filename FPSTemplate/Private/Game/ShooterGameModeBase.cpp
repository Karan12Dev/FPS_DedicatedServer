...
void AShooterGameModeBase::StartPlayerElimination(float ElimTime, ACharacter* ElimmedCharacter, APlayerController* VictimController, APlayerController* AttackerController)
{
...
void AShooterGameModeBase::PlayerEliminated(ACharacter* ElimmedCharacter, APlayerController* VictimController, APlayerController* AttackerController)
{
...
void AShooterGameModeBase::OnMatchEnded()
{
	...
	if (AMatchGameState* MatchGameState = GetGameState<AMatchGameState>(); IsValid(MatchGameState))
	{
		...
		{
			if (ADSPlayerController* DSPlayerController = Cast<ADSPlayerController>(Leader->GetPlayerController()); IsValid(DSPlayerController))
			{
				LeaderIds.Add(DSPlayerController->Username);
			}
		}
	}
